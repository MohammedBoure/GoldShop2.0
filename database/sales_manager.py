            ...
            with self.db.get_db_connection() as conn:
                ...
                query_vp = """
                    SELECT 
                        ...
                        0.0 as P_S,
                        IF(COALESCE(vp.montant_euro, 0) > 0 OR COALESCE(vp.or_casse_g, 0) > 0, 0.0, vp.montant_da) as Recette,
                        vp.or_casse_g as OC,
                        0.0 as TPE,
                        0.0 as Impos,
    ...
    def get_daily_totals(self, journee_id: int) -> dict:
        try:
            with self.db.get_db_connection() as conn:
                ...
                cursor.execute("""
                    SELECT 
                        SUM(CASE WHEN montant_da > 0 AND COALESCE(montant_euro, 0) = 0 AND COALESCE(or_casse_g, 0) = 0 THEN montant_da ELSE 0 END) as total_recette,
