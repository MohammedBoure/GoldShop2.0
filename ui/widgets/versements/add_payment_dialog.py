...
class AddPaymentDialog(QDialog):
    ...
    def open_discount_price_per_gram(self):
        ...
        pad = VirtualNumpad(
            title=f"Saisir le Prix/g Cible (Actuel: {unit_ppg:,.2f} DA/g)",
            mode="dialog",
            allow_decimal=True,
            allow_negative=False,
            initial_value=unit_ppg,
            parent=self
        )
        if pad.exec() == QDialog.Accepted:
            val = pad.get_value()
            if val:
                target_ppg = float(val)
                if target_ppg <= 0:
                    QMessageBox.warning(self, "Erreur", "Le prix/g cible doit être supérieur à 0.")
                    return
                target_ppg = min(target_ppg, unit_ppg)
                remise_val = max(0.0, (unit_ppg - target_ppg) * base_weight)
                self.inp_remise_da.setText(f"{remise_val:.2f}")
                self.auto_calculate_poids_deduit()
    ...
    def auto_calculate_poids_deduit(self):
        ...
        if base_amount > 0 and base_weight > 0:
            net_base = max(0.0, base_amount - remise)
            effective_ppg = net_base / base_weight if base_weight > 0 else 0.0
    ...
    def _sync_currency_or_casse_calculation(self, source_field, inp_qty, inp_rate, inp_total, qty_decimals=3):
        ...
        try:
            ...
            logging.error(f"[AddPaymentDialog] Erreur calcul devise/casse: {e}")
        finally:
